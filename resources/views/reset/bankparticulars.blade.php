<div class="form-card ">
    ...
    <div class="col-md-12 table-responsive">
        <table class="table table-bordered table-hover table-sortable" id="tab_logic">
            ...
            <tbody>
                @foreach ($client->bankParticulars()->get() as $key => $bankParticular)
                    <tr id='addr0' data-id="0" class="hidden" style="cursor: move;">
                        ...
                        <td data-name="AccountyType">
                            <select name="accountType[]" class="accountType">
                                <option value="" {{ $bankParticular->Account_type == '' ? 'selected' : '' }}>Select
                                    Option</option>
                                <option value="Individual"
                                    {{ $bankParticular->Account_type == 'Individual' ? 'selected' : '' }}>Individual
                                </option>
                                <option value="Joint" {{ $bankParticular->Account_type == 'Joint' ? 'selected' : '' }}>Joint
                                </option>
                            </select>
                        </td>
                            ...
                            <select type="text" name='branch[]' placeholder='Branch' class="form-control branch">
                                ...
                                @foreach ($branches as $key => $branch)
                                    <option value="{{ $branch->name }}"
                                        {{ $branch->name == $bankParticular->branch ? 'selected' : '' }}>{{ $branch->name }}
                                    </option>
                                @endforeach
                            </select>
                        </td>
