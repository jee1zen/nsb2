...
class OtpController extends Controller
{
    public function OTP(Request $request){
     ...
     $otp =  Otp::setValidity(1500)  // otp validity time in mins
       ...
       ->generate($email);
            ...
            Mail::send('emails.registerOtp', ['otp' => $otp->token], function($message) use($email){
                $message->to($email);
                $message->subject('Your OTP for NSB Registration');
            });
 ...
 //     ->validate($mobile, $otp->token);
      ...
      error_reporting(E_ALL);
        ...
        "number": "'.$mobile.'",
        "mask": "NSB FMC",
        "text": " Your OTP for transaction is '.$otp->token.' \n NSB Fund Management Co.Ltd",
     ...
     public function checkOTP(Request $request){
      $email = $request->email;
     $verify = Otp::validate($email, $request->otp);
       //  dd($verify);
       if($verify->status){
         $send['success'] = true;
